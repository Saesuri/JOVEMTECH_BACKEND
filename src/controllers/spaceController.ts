...
export const saveSpace = async (req: Request, res: Response) => {
  try {
    ...
    const { data, error } = await supabase
      .from('spaces')
      .upsert([{ id, floor_id, name, type, capacity, coordinates }])
      .select();
    ...
    await writeLog(userId, action, `${isUpdate ? 'Updated' : 'Created'} room: ${name}`);
...
export const getSpaces = async (req: Request, res: Response) => {
  try {
    ...
    const { data, error } = await supabase.from('spaces').select('*').eq('floor_id', floor_id);
    if (error) throw error;
    res.json(data);
  } catch (error: any) {
...
export const getAllSpacesGlobal = async (req: Request, res: Response) => {
  try {
    const { data, error } = await supabase.from('spaces').select('*, floors(name)').order('name');
    if (error) throw error;
    res.json(data);
  } catch (error: any) {
    res.status(500).json({ error: error.message });
...
export const updateSpace = async (req: Request, res: Response) => {
  try {
    ...
    // LOG
    await writeLog(userId, 'UPDATE_SPACE_DETAILS', `Updated details for room: ${name}`);
