...
export const getFloorStats = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const { data: spaces, error: spaceError } = await supabase.from('spaces').select('id').eq('floor_id', id);
...
export const deleteFloor = async (req: Request, res: Response) => {
  try {
    ...
    const { data: floor } = await supabase.from('floors').select('name').eq('id', id).single();
    const floorName = floor ? floor.name : 'Unknown Floor';

    // 1. Fetch spaces
    const { data: spaces } = await supabase.from('spaces').select('id').eq('floor_id', id);
    ...
    // 4. Delete floor
    const { error: floorError } = await supabase.from('floors').delete().eq('id', id);
    if (floorError) throw floorError;

    // LOG
    await writeLog(userId, 'DELETE_FLOOR', `Deleted floor: ${floorName} and all contents`);
