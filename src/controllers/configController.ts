...
export const getRoomTypes = async (req: Request, res: Response) => {
    const { data, error } = await supabase.from('room_types').select('*').order('label');
...
export const createRoomType = async (req: Request, res: Response) => {
    ...
    const { data, error } = await supabase.from('room_types').insert([{ value, label }]).select();
    if (error) return res.status(500).json({ error: error.message });
...
export const updateUserRole = async (req: Request, res: Response) => {
    ...
    await writeLog(userId, 'USER_ROLE_CHANGE', `Changed role of ${targetEmail} to ${role}`);
...
export const toggleRoomStatus = async (req: Request, res: Response) => {
    ...
    const { data, error } = await supabase.from('spaces').update({ is_active }).eq('id', id).select();
    if (error) return res.status(500).json({ error: error.message });

    const spaceName = data[0].name;
    await writeLog(userId, 'MAINTENANCE_TOGGLE', `Set ${spaceName} to ${is_active ? 'Active' : 'Maintenance'}`);
